...
class Order extends Model
{
    ...
    public function product_edits($kitchen_display_slack = '')
    {
        ...
        return $this->hasMany('App\Models\OrderProductLogs', 'order_id', 'id')
            ->leftJoin('products', 'products.id', '=', 'order_product_logs.product_id')
            ->select('order_product_logs.slack', 'order_product_logs.product_id', 'products.product_code', 'products.name', 'order_product_logs.quantity', 'order_product_logs.is_ready_to_serve', 'order_product_logs.kot_status', 'order_product_logs.edit_counter', 'order_product_logs.created_at')
            ->whereIn('products.category_id', $category_array)
