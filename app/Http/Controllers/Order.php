...
class Order extends Controller
{
    ...
    public function print_order(Request $request, $slack, $type = 'INLINE', $full_path = false)
    {
        ...
        if ($type == 'INLINE') {
            ...
            $upload_dir = Storage::disk('order')->path('');
    ...
    public function print_kot(Request $request, $slack, $type = 'INLINE', $full_path = false, $update_kot = false)
    {
        $data['menu_key'] = 'MM_ORDERS';
        $data['sub_menu_key'] = 'SM_POS_ORDERS';
        check_access([$data['sub_menu_key']]);
        ...
        if ($request->has('items')) {
            ...
            $custom_kot_product_array = (isset($order_data['product_edits']['products'])) ? $order_data['product_edits']['products'] : collect();
            $custom_kot_product_array = collect($custom_kot_product_array)->all();
            // dd($custom_kot_product_array);
            $kot_filtered = collect($custom_kot_product_array)->where('kot_status', '=', '0');
            // $kot_filtered = collect($custom_kot_product_array)->whereIn('slack', $items_array);

            $order_product['updated_at'] = now();
            $order_product['updated_by'] = $request->logged_user_id;
            $order_product['kot_status'] = 1;
            // update OrderProductLogs table
            $order_product_logs = OrderProductLogs::whereIn('slack', $kot_filtered->pluck('slack'))
                ->update($order_product);

            $custom_kot_products = $kot_filtered->toArray();
            if (count($custom_kot_products) == 0) {
                abort(404);
            }
        }
