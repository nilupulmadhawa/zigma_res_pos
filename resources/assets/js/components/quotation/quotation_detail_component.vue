    ...
    <div class="col-md-12">
          ...
          <div class="d-flex">
            <div>
              <span class="text-title">
                {{ $t("Quotation") }} #{{ quotation_basic.quotation_number }}
              </span>
            </div>
          </div>
        </div>
        <div class="">
          <span v-bind:class="quotation_basic.status.color">{{
            quotation_basic.status.label
          }}</span>
        </div>
      </div>
      ...
      <div class="d-flex flex-wrap mb-4" v-if="quotation_statuses != ''">
        ...
        <div class="ml-auto">
          <button
            ...
            v-on:click="printnode_print('QUOTATION')"
            v-bind:disabled="printing_processing == true"
          >
            <i
              class="fa fa-circle-notch fa-spin"
              v-if="printing_processing == true"
            ></i>
            {{ $t("Print") }}
          </button>
          ...
          <a
            class="btn btn-outline-primary mr-1"
            v-bind:href="'/print_quotation/' + slack"
            target="_blank"
            >{{ $t("PDF") }}</a
          >
          ...
          <button
            ...
            v-show="
              !block_delete_quotation.includes(quotation_basic.status.constant)
            "
            v-if="delete_quotation_access == true"
            v-on:click="delete_quotation()"
            v-bind:disabled="quotation_delete_processing == true"
          >
            <i
              class="fa fa-circle-notch fa-spin"
              v-if="quotation_delete_processing == true"
            ></i>
            {{ $t("Delete Quotation") }}
          </button>
          ...
          <div class="dropdown d-inline">
            ...
            <div
              ...
              aria-labelledby="quotation_action"
            >
              <button
                class="dropdown-item"
                type="button"
                v-for="(quotation_status, key, index) in quotation_statuses"
                v-bind:value="quotation_status.value_constant"
                v-bind:key="index"
                v-on:click="
                  change_quotation_status(quotation_status.value_constant)
                "
              >
                Mark as {{ quotation_status.label }}
              </button>
            </div>
          </div>
        </div>
      ...
      <div class="form-row mb-2">
        ...
        <div class="form-group col-md-3">
          ...
          <p>
            {{
              quotation_basic.created_by == null
                ? "-"
                : quotation_basic.created_by["fullname"] +
                  " (" +
                  quotation_basic.created_by["user_code"] +
                  ")"
            }}
          </p>
        </div>
        <div class="form-group col-md-3">
          ...
          <p>
            {{
              quotation_basic.updated_by == null
                ? "-"
                : quotation_basic.updated_by["fullname"] +
                  " (" +
                  quotation_basic.updated_by["user_code"] +
                  ")"
            }}
          </p>
        </div>
      ...
      <div class="mb-3">
        ...
        <div
          class="form-row mb-2"
          v-show="quotation_basic.bill_to == 'SUPPLIER'"
        >
          <div class="form-group col-md-3">
          ...
          <div class="form-group col-md-3">
            ...
            <p>
              {{
                quotation_basic.bill_to_email != null
                  ? quotation_basic.bill_to_email
                  : "-"
              }}
            </p>
          </div>
          <div class="form-group col-md-3">
            ...
            <p>
              {{
                quotation_basic.bill_to_contact != null
                  ? quotation_basic.bill_to_contact
                  : "-"
              }}
            </p>
          </div>
          <div class="form-group col-md-3">
            ...
            <p class="custom-pre">
              {{
                quotation_basic.bill_to_address != null
                  ? quotation_basic.bill_to_address
                  : "-"
              }}
            </p>
          </div>
        ...
        <div
          class="form-row mb-2"
          v-show="quotation_basic.bill_to == 'CUSTOMER'"
        >
          <div class="form-group col-md-3">
            <label for="bill_to">{{ $t("Bill To") }}</label>
            <p>{{ quotation_basic.bill_to }}</p>
          ...
          <div class="form-group col-md-3">
            ...
            <p>
              {{
                quotation_basic.bill_to_name != null
                  ? quotation_basic.bill_to_name
                  : "-"
              }}
            </p>
          </div>
        ...
        <div class="form-row mb-2">
          <div class="form-group col-md-3">
            ...
            <p>
              {{ quotation_basic.currency_name }} ({{
                quotation_basic.currency_code
              }})
            </p>
          </div>
        </div>
      ...
      <div class="table-responsive">
        <table class="table table-striped display nowrap text-nowrap w-100">
          <thead>
            <tr>
              ...
              <th
                ...
                class="text-right"
                v-show="tax_component_count > 0"
                v-for="(tax_component_array_item, index) in tax_component_array"
                v-bind:key="index"
              >
                {{ tax_component_array_item }} %
              </th>
          ...
          <tbody>
            <tr
              v-for="(quotation_product, key, index) in products"
              v-bind:value="quotation_product.product_slack"
              v-bind:key="index"
            >
              <th scope="row">{{ key + 1 }}</th>
              <td>
                {{
                  quotation_product.product_code
                    ? quotation_product.product_code
                    : "-"
                }}
              </td>
              ...
              <td class="text-right">
                {{ quotation_product.amount_excluding_tax }}
              </td>
              <td class="text-right">
                {{ quotation_product.discount_percentage }}
              </td>
              <td class="text-right">
                {{ quotation_product.discount_amount }}
              </td>
              <td class="text-right" v-show="tax_component_count == 0">
                {{ quotation_product.tax_percentage }}
              </td>
              <td
                class="text-right"
                v-show="tax_component_count > 0"
              ...
              >
                {{
                  quotation_product.tax_component_array[
                    tax_component_array_item
                  ]
                }}
              </td>
              ...
              <td class="text-right">
                {{ quotation_product.subtotal_amount_excluding_tax }}
              </td>
            </tr>
            <tr>
              <td v-bind:colspan="table_colspan" class="text-right">
                {{ $t("Sub Total") }} (EXCL Tax)
              </td>
              <td class="text-right">
                {{ quotation_basic.subtotal_excluding_tax }}
              </td>
            </tr>
            <tr>
              <td v-bind:colspan="table_colspan" class="text-right">
              ...
              <td class="text-right">
                {{ quotation_basic.total_discount_amount }}
              </td>
            </tr>
            <tr>
              <td v-bind:colspan="table_colspan" class="text-right">
              ...
              <td class="text-right">
                {{ quotation_basic.total_after_discount }}
              </td>
            </tr>
            <tr>
              <td v-bind:colspan="table_colspan" class="text-right">
            ...
            <tr>
              ...
              <td class="text-right">
                {{ quotation_basic.total_order_amount }}
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="form-row mb-2">
        <div class="form-group col-md-12">
          <p class="custom-pre">
            {{ quotation_basic.notes != null ? quotation_basic.notes : "-" }}
          </p>
        </div>
      </div>
    </div>
...
export default {
  data() {
    return {
      ...
      change_quotation_link:
        "/api/update_quotation_status/" + this.quotation_data.slack,
      delete_quotation_api_link:
        "/api/delete_quotation/" + this.quotation_data.slack,
      ...
      tax_component_count:
        this.quotation_data.tax_option_data != null
          ? this.quotation_data.tax_option_data.component_count
          : 0,
      tax_component_array:
        this.quotation_data.tax_option_data != null
          ? this.quotation_data.tax_option_data.component_array
          : [],
      table_colspan:
        parseInt(8) +
        (this.quotation_data.tax_option_data != null
          ? parseInt(this.quotation_data.tax_option_data.component_count)
          : 1),
  ...
  methods: {
    change_quotation_status(quotation_status) {
      ...
      axios
        .post(this.change_quotation_link, formData)
        .then((response) => {
    ...
    delete_quotation() {
      ...
      this.$on("submit", function () {
        ...
        axios
          .post(this.delete_quotation_api_link, formData)
          .then((response) => {
            if (response.data.status_code == 200) {
              if (
          ...
          if (response.data.status_code == 200) {
            axios
              .post("http://localhost:8191/print", {
                pdf_base64: response.data.data.content ?? "",
                printer_name: response.data.data.printer,
              })
              .then((response) => {
                this.show_response_message("Printed Successfully", "SUCCESS");
