<template>
  <div class="row">
    <div class="col-md-12">
          ...
          <div class="d-flex">
            <div>
              <span class="text-title">
                {{ $t("Purchase Order") }} #{{ po_basic.po_number }}
              </span>
            </div>
          </div>
        </div>
        <div class="">
          <span v-bind:class="po_basic.status.color">{{
            po_basic.status.label
          }}</span>
        </div>
      </div>
      ...
      <div class="d-flex flex-wrap mb-4" v-if="po_statuses != ''">
        ...
        <div class="ml-auto">
          <button
            ...
            v-on:click="printnode_print('PURCHASE_ORDER')"
          ...
          <a
            class="btn btn-outline-primary mr-1"
            v-bind:href="'/print_purchase_order/' + slack"
            target="_blank"
            >{{ $t("PDF") }}</a
          >
          ...
          <button
            type="button"
            class="btn btn-outline-primary mr-1"
            v-if="create_invoice_from_po_access == true"
            v-on:click="generate_invoice()"
            v-bind:disabled="generate_invoice_processing == true"
          >
            <i
              class="fa fa-circle-notch fa-spin"
              v-if="generate_invoice_processing == true"
            ></i>
            {{ $t("Generate Invoice") }}
          </button>
          ...
          <button
            ...
            v-show="!block_delete_po.includes(po_basic.status.constant)"
            v-if="delete_po_access == true"
            v-on:click="delete_po()"
            v-bind:disabled="po_delete_processing == true"
          >
            <i
              class="fa fa-circle-notch fa-spin"
              v-if="po_delete_processing == true"
            ></i>
            {{ $t("Delete Purchase Order") }}
          </button>
          ...
          <div class="dropdown d-inline">
            ...
            >
              <button
                class="dropdown-item"
                type="button"
                v-for="(po_status, key, index) in po_statuses"
                v-bind:value="po_status.value_constant"
                v-bind:key="index"
                v-on:click="change_po_status(po_status.value_constant)"
              >
                Mark as {{ po_status.label }}
              </button>
            </div>
          </div>
        </div>
      ...
      <div class="form-row mb-2">
        ...
        <div class="form-group col-md-3">
          ...
          <p>
            {{
              po_basic.created_by == null
                ? "-"
                : po_basic.created_by["fullname"] +
                  " (" +
                  po_basic.created_by["user_code"] +
                  ")"
            }}
          </p>
        </div>
        <div class="form-group col-md-3">
          ...
          <p>
            {{
              po_basic.updated_by == null
                ? "-"
                : po_basic.updated_by["fullname"] +
                  " (" +
                  po_basic.updated_by["user_code"] +
                  ")"
            }}
          </p>
        </div>
      ...
      <div v-if="po_basic.invoices != null && po_basic.invoices.length != 0">
        ...
        <div class="d-flex flex-wrap mb-4">
          <span
            v-for="(invoice, key, index) in po_basic.invoices"
            v-bind:value="invoice.slack"
            v-bind:key="index"
          >
            <span v-if="invoice.detail_link != ''"
              ><a v-bind:href="invoice.detail_link" target="_blank">{{
                invoice.invoice_number
              }}</a></span
            ><span v-else>{{ invoice.invoice_number }}</span>
            &nbsp;&middot;&nbsp;
          </span>
        </div>
      </div>
      ...
      <div class="table-responsive">
        <table class="table table-striped display nowrap text-nowrap w-100">
          ...
          <tbody>
            <tr
              v-for="(po_product, key, index) in products"
              v-bind:value="po_product.product_slack"
              v-bind:key="index"
            >
              <th scope="row">{{ key + 1 }}</th>
              <td>
                {{ po_product.product_code ? po_product.product_code : "-" }}
              </td>
              <td>{{ po_product.name }}</td>
              <td class="text-right">
                {{ po_product.quantity }}
                <span v-if="po_product.stock_update == 1" class="d-block small"
                  ><i class="fas fa-check text-success"></i> Added to
                  Stock</span
                >
              </td>
              ...
              <td
                ...
                v-bind:key="index"
              >
                {{ po_product.tax_component_array[tax_component_array_item] }}
              </td>
              <td class="text-right">{{ po_product.tax_amount }}</td>
              <td class="text-right">
                {{ po_product.subtotal_amount_excluding_tax }}
              </td>
            </tr>
            <tr>
      ...
      <div class="form-row mb-2">
        <div class="form-group col-md-6">
          <p class="custom-pre">
            {{ po_basic.terms != null ? po_basic.terms : "-" }}
          </p>
        </div>
      </div>
    </div>
    ...
    <modalcomponent
      v-if="show_generate_invoice_modal"
      v-on:close="show_generate_invoice_modal = false"
    >
      <template v-slot:modal-header>
        {{ $t("Confirm") }}
      </template>
...
export default {
  data() {
    return {
      ...
      delete_po_api_link:
        "/api/delete_purchase_order/" + this.purchase_order_data.slack,
      generate_invoice_from_po_api_link:
        "/api/generate_invoice_from_po/" + this.purchase_order_data.slack,
      ...
      tax_component_count:
        this.purchase_order_data.tax_option_data != null
          ? this.purchase_order_data.tax_option_data.component_count
          : 0,
      tax_component_array:
        this.purchase_order_data.tax_option_data != null
          ? this.purchase_order_data.tax_option_data.component_array
          : [],
      table_colspan:
        parseInt(8) +
        (this.purchase_order_data.tax_option_data != null
          ? parseInt(this.purchase_order_data.tax_option_data.component_count)
          : 1),
  ...
  methods: {
    change_po_status(po_status) {
      ...
      axios
        .post(this.change_po_link, formData)
        .then((response) => {
    ...
    delete_po() {
      ...
      this.$on("submit", function () {
        ...
        axios
          .post(this.delete_po_api_link, formData)
          .then((response) => {
            if (response.data.status_code == 200) {
              if (
    ...
    generate_invoice() {
      ...
      this.$on("submit", function () {
        ...
        axios
          .post(this.generate_invoice_from_po_api_link, formData)
          .then((response) => {
            if (response.data.status_code == 200) {
