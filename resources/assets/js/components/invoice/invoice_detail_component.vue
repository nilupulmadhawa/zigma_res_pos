    ...
    <div class="col-md-12">
          ...
          <div class="d-flex">
            <div>
              <span class="text-title">
                {{ $t("Invoice") }} #{{ invoice_basic.invoice_number }}
              </span>
            </div>
          </div>
        </div>
      ...
      <div class="d-flex flex-wrap mb-4">
        ...
        <div class="ml-auto">
          <button
            ...
            v-on:click="printnode_print('INVOICE')"
          ...
          <a
            class="btn btn-outline-primary mr-1"
            v-bind:href="'/print_invoice/' + slack"
            target="_blank"
            >{{ $t("PDF") }}</a
          >
          ...
          <button
            ...
            v-show="
              !block_delete_invoice.includes(invoice_basic.status.constant)
            "
            v-if="delete_invoice_access == true"
            v-on:click="delete_invoice()"
            v-bind:disabled="invoice_delete_processing == true"
          >
            <i
              class="fa fa-circle-notch fa-spin"
              v-if="invoice_delete_processing == true"
            ></i>
            {{ $t("Delete Invoice") }}
          </button>
          ...
          <button
            type="submit"
            class="btn btn-outline-primary mr-1"
            v-on:click="show_payment_modal = true"
            v-show="!block_make_payment.includes(invoice_basic.status.constant)"
            v-if="make_payment_access == true"
          >
            {{ $t("Record Payment") }}
          </button>
          ...
          <div class="dropdown d-inline" v-if="invoice_statuses != ''">
            ...
            <div
              ...
              aria-labelledby="invoice_action"
            >
              <button
                class="dropdown-item"
                type="button"
                v-for="(invoice_status, key, index) in invoice_statuses"
                v-bind:value="invoice_status.value_constant"
                v-bind:key="index"
                v-on:click="
                  change_invoice_status(invoice_status.value_constant)
                "
              >
                Mark as {{ invoice_status.label }}
              </button>
            </div>
          </div>
        </div>
      ...
      <div class="form-row mb-2">
        ...
        <div class="form-group col-md-3">
          ...
          <p>
            {{
              invoice_basic.created_by == null
                ? "-"
                : invoice_basic.created_by["fullname"] +
                  " (" +
                  invoice_basic.created_by["user_code"] +
                  ")"
            }}
          </p>
        </div>
        <div class="form-group col-md-3">
          ...
          <p>
            {{
              invoice_basic.updated_by == null
                ? "-"
                : invoice_basic.updated_by["fullname"] +
                  " (" +
                  invoice_basic.updated_by["user_code"] +
                  ")"
            }}
          </p>
        </div>
      ...
      <div class="mb-3">
        ...
        <div class="form-row mb-2" v-show="invoice_basic.bill_to == 'SUPPLIER'">
          ...
          <div class="form-group col-md-3">
            ...
            <p>
              {{
                invoice_basic.bill_to_email != null
                  ? invoice_basic.bill_to_email
                  : "-"
              }}
            </p>
          </div>
          <div class="form-group col-md-3">
            ...
            <p>
              {{
                invoice_basic.bill_to_contact != null
                  ? invoice_basic.bill_to_contact
                  : "-"
              }}
            </p>
          </div>
          <div class="form-group col-md-3">
            ...
            <p class="custom-pre">
              {{
                invoice_basic.bill_to_address != null
                  ? invoice_basic.bill_to_address
                  : "-"
              }}
            </p>
          </div>
        ...
        <div class="form-row mb-2" v-show="invoice_basic.bill_to == 'CUSTOMER'">
          ...
          <div class="form-group col-md-3">
            ...
            <p>
              {{
                invoice_basic.bill_to_name != null
                  ? invoice_basic.bill_to_name
                  : "-"
              }}
            </p>
          </div>
        ...
        <table class="table table-striped display nowrap text-nowrap w-100">
          ...
          <tbody>
            ...
            >
              ...
              >
                {{
                  invoice_product.tax_component_array[tax_component_array_item]
                }}
              </td>
              <td class="text-right">{{ invoice_product.tax_amount }}</td>
              <td class="text-right">
                {{ invoice_product.subtotal_amount_excluding_tax }}
              </td>
            </tr>
            <tr>
      ...
      <div class="form-row mb-2">
        <div class="form-group col-md-6">
          <p class="custom-pre">
            {{ invoice_basic.terms != null ? invoice_basic.terms : "-" }}
          </p>
        </div>
      </div>
    ...
    >
      ...
      <template v-slot:modal-body>
        <addtransactionwidgetcomponent
          ...
          :invoice_slack="slack"
          :bill_to_prop="'INVOICE'"
          :currency_codes="currency_codes"
          :default_transaction_type="default_transaction_type"
        ></addtransactionwidgetcomponent>
      </template>
      <template v-slot:modal-footer>
        <button type="button" class="btn btn-light" @click="cancel_transaction">
          Cancel
        </button>
        <button
          type="button"
          ...
          v-show="show_payment_submit"
          v-bind:disabled="make_payment_processing == true"
        >
          <i
            class="fa fa-circle-notch fa-spin"
            v-if="make_payment_processing == true"
          ></i>
          Continue
        </button>
      </template>
...
export default {
  data() {
    return {
      ...
      change_invoice_link:
        "/api/update_invoice_status/" + this.invoice_data.slack,
      ...
      tax_component_count:
        this.invoice_data.tax_option_data != null
          ? this.invoice_data.tax_option_data.component_count
          : 0,
      tax_component_array:
        this.invoice_data.tax_option_data != null
          ? this.invoice_data.tax_option_data.component_array
          : [],
      table_colspan:
        parseInt(8) +
        (this.invoice_data.tax_option_data != null
          ? parseInt(this.invoice_data.tax_option_data.component_count)
          : 1),
  ...
  methods: {
    change_invoice_status(invoice_status) {
      ...
      axios
        .post(this.change_invoice_link, formData)
        .then((response) => {
    ...
    delete_invoice() {
      ...
      this.$on("submit", function () {
        ...
        axios
          .post(this.delete_invoice_api_link, formData)
          .then((response) => {
            if (response.data.status_code == 200) {
              if (
