<template>
  <div class="">
      ...
      <div class="d-flex flex-wrap p-4 border-bottom">
        ...
        <div class="ml-auto">
          <button
            ...
            v-on:click="show_kot_modal = true"
          >
            {{ $t("Custom KOT") }}
          </button>
    ...
    <modalcomponent
      v-if="show_kot_modal"
      v-on:close="show_kot_modal = false"
    ...
    >
      ...
      <template v-slot:modal-body>
        ...
        <div
          class="d-flex flex-column pt-2 pb-2 mb-2"
          v-for="(products, datetime) in order_basic.product_edits
            .items_grouped_by_edit_time"
          v-bind:key="datetime"
        >
          <div class="d-flex justify-content-end">
            <span class="small">{{
              humanize_duration(calculate_duration(datetime))
            }}</span>
          </div>
          <div
            class="d-flex justify-content-between align-items-center mb-1 cursor"
          ...
          >
            <div class="mr-2">
              <input
                class=""
                type="checkbox"
                v-model="kot_products"
                v-bind:value="product.slack"
                v-bind:id="product.slack"
              />
            </div>
            <div class="mr-auto">
              <label v-bind:for="product.slack" class="font-weight-normal">
                ...
                <span
                  ...
                  v-if="product.is_ready_to_serve == 1"
                  ><i class="fas fa-check-circle"></i
                ></span>
              </label>
            </div>
            <div class="text-right w-25" v-bind:for="product.slack">
              {{ product.quantity }}
            </div>
          </div>
        </div>
      ...
      <template v-slot:modal-footer>
        ...
        <button
          ...
          :disabled="kot_products.length == 0"
          v-if="order_detail_access == true && print_kot_link != ''"
          @click="custom_kot_pdf()"
        >
          {{ $t("PDF") }}
        </button>
        <button
          ...
          v-if="
            ...
            printnode_enabled == true
          "
          :disabled="kot_products.length == 0 || custom_kot_processing == true"
          @click="printnode_print('CUSTOM_KOT')"
        >
          <i
            class="fa fa-circle-notch fa-spin"
            v-if="custom_kot_processing == true"
          ></i>
          {{ $t("Print KOT") }}
        </button>
      </template>
...
export default {
  ...
  methods: {
    ...
    custom_kot_pdf() {
      window.open(
        this.print_kot_link + "?items=" + this.kot_products,
        "_blank"
      );
    },
  },
