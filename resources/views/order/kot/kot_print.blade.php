...
<html>
<head>
    <title>Order {{ $print_title }} #{{ $data->order_number }}</title>
</head>
<body>
    <div class='center border-bottom-dashed pb-1rem'>
        <div class='bold display-block font-large'>{{ $print_title }}</div>
    </div>
    <div class='border-bottom-dashed pt-1rem mb-1rem'>
        <table class='w-100'>
            ...
            @if ($data->restaurant_mode == 1)
                ...
                @if ($data->waiter_data != '')
                    <tr>
                        <td class='' colspan="2">Waiter:
                            {{ $data->waiter_data != '' ? $data->waiter_data->user_code . '-' . $data->waiter_data->fullname : '-' }}
                        </td>
                    </tr>
                @endif
            @endif
    ...
    <table class='border-bottom-dashed mb-1rem w-100 page-break-avoid'>
        ...
        @foreach ($products as $order_products)
            ...
            @endphp
            <tr class="">
                <td class='{{ $addon_spacing }} pb-1rem'>
