...
<body>
    <div class='center border-bottom-dashed pb-1rem'>
        <div class='bold display-block font-large'>{{ $setting_app->company_name }}</div>
        {{-- <div class='bold display-block font-large'>{{ $data->store->name }}</div> --}}
        <div>
            {{ $data->store->address }}
    ...
    <table class='border-bottom-dashed mb-1rem w-100 table-extended page-break-avoid'>
        ...
        @foreach ($data->products as $order_products)
            ...
            @endphp
            <tr>
                <td class='{{ $spacing . $addon_spacing }}'>
                    {{ $addon_indicator }}{{ $order_products->product_code . '-' . $order_products->name }}</td>
        ...
        @if ($data->additional_discount_percentage > 0)
            <tr>
                ...
                <td class='right'>({{ $data->additional_discount_percentage }}%)
                    {{ $data->additional_discount_amount }}</td>
            </tr>
        @endif
        <tr>
    ...
    @if (isset($data->order_type_data) &&
            $data->order_type_data->order_type_constant == 'DELIVERY' &&
